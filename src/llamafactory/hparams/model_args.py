...
class ModelArguments(QuantizationArguments, ProcessorArguments, ExportArguments, VllmArguments):
    ...
    recurrent_model: bool = field(
        ...
        metadata={
            "help": (
                "Enable the pondering forward pass. "
                "Required for PonderLM-2 (`--patch_method ours`), Loop, Pause and PonderLM. "
                "Set to False for the vanilla transformer baseline."
            )
        },
    )
    scale_embeds: bool = field(
        default=False,
        metadata={"help": "Scale token embeddings by sqrt(hidden_size) before pondering (recommended)."},
    )
    ...
    num_latent_thoughts: int = field(
        ...
        metadata={
            "help": (
                "Number of additional ponder iterations. "
                "PonderLM-2: number of initial interpolation stages (= K in the paper). "
                "Loop: number of extra full passes through the layer stack. "
                "Pause Token: number of pause tokens inserted after each input token. "
                "PonderLM (baseline): number of pondering forward passes."
            )
        },
    )
    num_jacobi_iterations: int = field(
        default=3,
        metadata={"help": "PonderLM-2 only: number of Jacobi refinement steps. Used at both train and eval."},
    )
    random_jacobi_iterations: bool = field(
        default=False,
        metadata={
            "help": "PonderLM-2 only: randomise the per-step training count in [2, 4) (curriculum used in the paper)."
        },
    )
    interpolation: bool = field(
        ...
        metadata={
            "help": (
                "Use softmax-weighted token-embedding interpolation between iterations. "
                "If False, the raw last-hidden-state is fed back as the next-stage embedding."
            )
        },
    )
    ...
    patch_method: Optional[Literal["ours", "ponderlm", "loop", "pause", "vanilla"]] = field(
        default=None,
        metadata={
            "help": (
                "Which pondering implementation to swap in. "
                "`ours` = PonderLM-2 (this paper). "
                "`ponderlm`, `loop`, `pause` = baselines from related work. "
                "`vanilla` = standard Llama / GPT-NeoX with no pondering. "
                "When unset, the underlying transformers model is left untouched."
            )
        },
    )
