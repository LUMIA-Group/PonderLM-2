...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.checkpoint_num_layers = getattr(config, "checkpoint_num_layers", 100)
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    @staticmethod
    def _calculate_mse_embedding_change(new_embedding: torch.Tensor, previous_embedding: torch.Tensor):
        diff = new_embedding - previous_embedding
        mse_change = (diff * diff).mean(dtype=torch.float32)
        prev_energy = (previous_embedding * previous_embedding).mean(dtype=torch.float32)
        relative_mse_change = mse_change / prev_energy.clamp_min(1e-9)
        return mse_change, relative_mse_change
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            if past_key_values is not None:
                ...
                if is_decoding:
                    ...
                    weight_for_interpolation = self.model.embed_tokens.weight
                    ...
                    num_decode_stages = int(getattr(self.config, 'num_latent_thoughts', 0)) + 1
                    stage_inputs = inputs_embeds0
                    current_past_key_values = past_key_values
                    current_attention_mask = attention_mask
                    last_stage_outputs = None

                    for stage_idx in range(num_decode_stages):
                        is_final_stage = (stage_idx == num_decode_stages - 1)
                        last_stage_outputs = self.model(
                            inputs_embeds=stage_inputs,
                            attention_mask=current_attention_mask,
                            position_ids=position_ids,
                            past_key_values=current_past_key_values,
                            use_cache=True,
                            output_attentions=output_attentions if is_final_stage else False,
                            output_hidden_states=user_requested_output_hidden_states if is_final_stage else False,
                            return_dict=True,
                            cache_position=cache_position,
                        )

                        current_past_key_values = last_stage_outputs.past_key_values

                        if not is_final_stage:
                            if self.config.interpolation == True:
                                stage_inputs = self._compute_interpolated_embeds(
                                    weight_for_interpolation,
                                    last_stage_outputs.last_hidden_state,
                                    use_topk=getattr(self.config, 'interpolation_use_topk', False),
                                )
                            else:
                                stage_inputs = last_stage_outputs.last_hidden_state
                    ...
                    logits = self.lm_head(last_stage_outputs.last_hidden_state)

                    if not return_dict:
                        return (logits, last_stage_outputs.past_key_values)

                    return CausalLMOutputWithPast(
                        loss=None,
                        logits=logits,
                        past_key_values=last_stage_outputs.past_key_values,
                        hidden_states=last_stage_outputs.hidden_states,
                        attentions=last_stage_outputs.attentions,
                    )
                    ...
                    for iter_k_idx in range(num_initial_interpolation_stages): 
                        ...
                        iter_gpt_neox_outputs = self.model(
                    ...
                    if num_initial_interpolation_stages > 0 and num_refinement_steps > 0:
                        for ref_idx in range(num_refinement_steps):
                            ...
                            ref_gpt_neox_outputs = self.model(
                            ...
                            for i in range(num_initial_interpolation_stages):
                                ...
                                if previous_refined_embedding_for_mse is not None:
                                    mse_change, relative_mse_change = self._calculate_mse_embedding_change(
                                        refined_embeds_for_stage,
                                        previous_refined_embedding_for_mse
                                    )
                                    if os.getenv("LLAMA_WITHCACHE_DEBUG_REFINE", "0") == "1":
                                        print(
                                            f"Refinement Iteration {ref_idx + 1}: "
                                            f"MSE change for the refined embedding = {mse_change.item()}, "
                                            f"Relative MSE = {relative_mse_change.item()}"
                                        )
                    ...
                    logits = self.lm_head(final_pass_outputs.last_hidden_state[:,-1:,:])
            ...
            # For interpolation and refinement, we need hidden states.
                ...
                num_initial_interpolation_stages = getattr(self.config, 'num_latent_thoughts', 0)
                all_embeddings_stages: List[torch.Tensor] = [scaled_inputs_embeds0_for_orig_slots]
                ...
                if self.training and getattr(self.config, "random_jacobi_iterations", False):
                    ...
                    num_refinement_steps = getattr(self.config, 'num_jacobi_iterations', 3)
