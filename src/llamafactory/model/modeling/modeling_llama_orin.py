    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            # 将初始 embedding 预乘 num_latent_thoughts
            current_embeds = inputs_embeds0 #* self.config.num_latent_thoughts

            # 迭代优化 embedding
            for iteration in range(self.config.num_latent_thoughts):
                outputs = self.model(
                    input_ids=None,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                ...
                if self.config.output_hidden_states and getattr(self.config, "hidden_layer_num", None):
                    if getattr(self.config, "high_memory_mode", False):
                        ...
                        selected_layers = [outputs.hidden_states[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                        probs = sum(
                            torch.softmax(self.lm_head(layer), dim=-1)
