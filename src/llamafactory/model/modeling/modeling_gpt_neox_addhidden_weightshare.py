    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module, '_is_attention_output') or hasattr(module, '_is_mlp_output'):
                ...
                # std = 2 / self.config.num_hidden_layers / math.sqrt(self.config.hidden_size)/(self.config.num_latent_thoughts + 1)
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        if hasattr(self.config, 'recurrent_model') and self.config.recurrent_model:
            ...
            num_latent_thoughts = getattr(self.config, 'num_latent_thoughts', 0)
            
            for repeat in range(num_latent_thoughts + 1):
